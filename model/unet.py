...
def build_vgg_model(inputs):
    with slim.arg_scope(vgg.vgg_arg_scope()):
        _, end_points = vgg.vgg_19(inputs, is_training=False)
    return end_points

def get_feature_maps(inputs):
    preprocessed = inputs * 255.0 - [123.68, 116.779, 103.939]  # VGG preprocessing
    with slim.arg_scope(vgg.vgg_arg_scope()):
        with tf.variable_scope('vgg_19', reuse=tf.AUTO_REUSE):
            _, end_points = vgg.vgg_19(preprocessed, is_training=False)
    for key in end_points.keys():
        print(key)
    return end_points


def perceptual_loss(target_image, generated_image):
    target_features = get_feature_maps(target_image)
    generated_features = get_feature_maps(generated_image)
    
    loss = 0
    for layer in ['vgg_19/vgg_19/conv3/conv3_3', 'vgg_19/vgg_19/conv4/conv4_3']:  # 使用檢查到的層名稱
        target_feature = target_features[layer]
        generated_feature = generated_features[layer]
        loss += tf.reduce_mean(tf.square(target_feature - generated_feature))
    return loss
...
class UNet(object):
    def __init__(self, experiment_dir=None, experiment_id=0, batch_size=16, input_width=256, output_width=256,
        ...
        self.vgg_input = tf.placeholder(tf.float32, shape=[None, 224, 224, 3], name='vgg_input')
        self.vgg_model = build_vgg_model(self.vgg_input)


    def self_attention(self, x, name):
        with tf.variable_scope(name):
            f = conv2d(x, x.get_shape()[-1] // 8, kh=1, kw=1, sh=1, sw=1, scope='f_conv')  # [bs, h, w, c']
            g = conv2d(x, x.get_shape()[-1] // 8, kh=1, kw=1, sh=1, sw=1, scope='g_conv')  # [bs, h, w, c']
            h = conv2d(x, x.get_shape()[-1], kh=1, kw=1, sh=1, sw=1, scope='h_conv')      # [bs, h, w, c]

            s = tf.matmul(tf.reshape(g, [x.get_shape()[0], -1, g.get_shape()[-1]]),
                          tf.reshape(f, [x.get_shape()[0], -1, f.get_shape()[-1]]),
                          transpose_b=True)  # [bs, N, N]

            beta = tf.nn.softmax(s)  # attention map

            o = tf.matmul(beta, tf.reshape(h, [x.get_shape()[0], -1, h.get_shape()[-1]]))  # [bs, N, C]
    ...
    def resnet_block(self, x, dim, scope):
        with tf.variable_scope(scope):
            y = conv2d(x, output_filters=dim, kh=3, kw=3, sh=1, sw=1, scope='c1')
            y = batch_norm(y, is_training=True, scope='bn1')
            y = tf.nn.relu(y)
            y = conv2d(y, output_filters=dim, kh=3, kw=3, sh=1, sw=1, scope='c2')
            y = batch_norm(y, is_training=True, scope='bn2')

            # 如果輸入和輸出的通道數不同，使用 1x1 卷積進行調整
            if x.get_shape()[-1] != dim:
                x = conv2d(x, output_filters=dim, kh=1, kw=1, sh=1, sw=1, scope='c3')

            return y + x

    def encode_layer(self, x, output_filters, layer, encode_layers, is_training):
        act = lrelu(x)
        conv = conv2d(act, output_filters=output_filters, kh=3, kw=3, sh=2, sw=2, scope="g_e%d_conv" % layer)
        enc = batch_norm(conv, is_training, scope="g_e%d_bn" % layer)
        encode_layers["e%d" % layer] = enc
        # 添加殘差塊
        if layer in [3, 5, 7]:  # 在特定層後添加殘差塊
            enc = self.resnet_block(enc, output_filters, scope="g_e%d_resblock" % layer)
        return enc

    def decode_layer(self, x, output_width, output_filters, layer, enc_layer, ids, inst_norm, is_training, dropout=False, do_concat=True):
        ...
        # 添加殘差塊
        if layer in [4, 6, 8]:  # 在特定層後添加殘差塊
            dec = self.resnet_block(dec, output_filters, scope="g_d%d_resblock" % layer)
        return dec
    ...
    def encoder(self, images, is_training, reuse=False):
        with tf.variable_scope("generator"):
            ...
            e1 = conv2d(images, self.generator_dim, kh=3, kw=3, sh=2, sw=2, scope="g_e1_conv")
            encode_layers["e1"] = e1
            e2 = self.encode_layer(e1, self.generator_dim * 2, 2, encode_layers, is_training)
            e3 = self.encode_layer(e2, self.generator_dim * 4, 3, encode_layers, is_training)
            e4 = self.encode_layer(e3, self.generator_dim * 8, 4, encode_layers, is_training)
            
             # 在特定層之後添加自注意力層
            e4 = self.self_attention(e4, name='self_attention_e4')
        
            e5 = self.encode_layer(e4, self.generator_dim * 8, 5, encode_layers, is_training)
            e6 = self.encode_layer(e5, self.generator_dim * 8, 6, encode_layers, is_training)
            e7 = self.encode_layer(e6, self.generator_dim * 8, 7, encode_layers, is_training)
            e8 = self.encode_layer(e7, self.generator_dim * 8, 8, encode_layers, is_training)
    ...
    def decoder(self, encoded, encoding_layers, ids, inst_norm, is_training, reuse=False):
        with tf.variable_scope("generator"):
            ...
            d1 = self.decode_layer(encoded, s128, self.generator_dim * 8, layer=1, enc_layer=encoding_layers["e7"], ids=ids, inst_norm=inst_norm, is_training=is_training, dropout=True)
            d2 = self.decode_layer(d1, s64, self.generator_dim * 8, layer=2, enc_layer=encoding_layers["e6"], ids=ids, inst_norm=inst_norm, is_training=is_training, dropout=True)
            d3 = self.decode_layer(d2, s32, self.generator_dim * 8, layer=3, enc_layer=encoding_layers["e5"], ids=ids, inst_norm=inst_norm, is_training=is_training, dropout=True)
            d4 = self.decode_layer(d3, s16, self.generator_dim * 8, layer=4, enc_layer=encoding_layers["e4"], ids=ids, inst_norm=inst_norm, is_training=is_training)
            
            # 在特定層之後添加自注意力層
            d4 = self.self_attention(d4, name='self_attention_d4')
        
            d5 = self.decode_layer(d4, s8, self.generator_dim * 4, layer=5, enc_layer=encoding_layers["e3"], ids=ids, inst_norm=inst_norm, is_training=is_training)
            d6 = self.decode_layer(d5, s4, self.generator_dim * 2, layer=6, enc_layer=encoding_layers["e2"], ids=ids, inst_norm=inst_norm, is_training=is_training)
            d7 = self.decode_layer(d6, s2, self.generator_dim, layer=7, enc_layer=encoding_layers["e1"], ids=ids, inst_norm=inst_norm, is_training=is_training)
            d8 = self.decode_layer(d7, s, self.output_filters, layer=8, enc_layer=None, ids=ids, inst_norm=inst_norm, is_training=is_training, do_concat=False)
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        g_loss = cheat_loss + l1_loss + self.Lcategory_penalty * fake_category_loss + const_loss + tv_loss
         ...
         # perceptual loss
        real_B_resized = tf.image.resize_images(real_B, [224, 224])
        fake_B_resized = tf.image.resize_images(fake_B, [224, 224])
        perceptual_loss_value = perceptual_loss(real_B_resized, fake_B_resized)

        d_loss = d_loss_real + d_loss_fake + category_loss / 2.0
        g_loss = cheat_loss + l1_loss + self.Lcategory_penalty * fake_category_loss + const_loss + tv_loss + perceptual_loss_value   
...
#                                              g_loss_summary, tv_loss_summary])
        ...
        fake_category_loss_summary = tf.summary.scalar("fake_category_loss", fake_category_loss)
        const_loss_summary = tf.summary.scalar("const_loss", const_loss)
        perceptual_loss_summary = tf.summary.scalar("perceptual_loss", perceptual_loss_value)
        d_loss_summary = tf.summary.scalar("d_loss", d_loss)
        g_loss_summary = tf.summary.scalar("g_loss", g_loss)
        tv_loss_summary = tf.summary.scalar("tv_loss", tv_loss)
